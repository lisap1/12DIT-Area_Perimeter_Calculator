...
def calculations(formula, result, substitutions, history_placement, unit):
    if area_perimeter == "Perimeter":
        product = formula.subs({x: substitutions[0], y: substitutions[1],
                                z: substitutions[2]})
    else:
        product = formula.subs({x: substitutions[0], y: substitutions[1],
                                z: substitutions[2],
...
display_instructions = in_list("Display instructions?(enter yes or no) : ",
                               yes_no, "Error - Please enter yes or no")
print("======================================================================"
      "=======")
# Start loop
shape = ''
while shape != 'Xxx':
    ...
    print()
    shape = in_list("Select shape: ", valid_shapes, "Error - Please enter "
                                                    "rectangle, circle, "
    ...
    if shape == 'Triangle' or shape == 'Parallelogram':
        ...
        if display_instructions == "Yes":
            print()
            print("Enter 'area' to calculate area of the " + shape.lower() +
                  ", or 'perimeter' for perimeter\n"
                  "Shortcuts: 'a' for area, 'p' for perimeter")
        # check for correct area/perimeter input
        area_perimeter = in_list("Calculate area or perimeter?: ",
                                 valid_area_perimeter,
                                 "Error - Please enter either area or "
                                 "perimeter")
    ...
    if area_perimeter != 'Xxx':
        ...
        if display_instructions == "Yes":
            print()
            print("Enter the dimensions according to the diagram. e.g. 4, 78, "
                  "3.56\nYou can enter any number between 0.01 - 999, "
                  "including decimals",)
        print()
        print(shape_ascii[shape])
        # asks user for the dimensions of their shape
        dimension_history = dimension_q(subtract, "Error - Please enter a "
                                                  "number between 0.01 and "
                                                  "999")
        # if the user exited the program during dimension input, scrap the
        # previous inputs
        if dimension_history == 'xxx':
            ...
            if display_instructions == "Yes":
                ...
                print("Enter a unit of measurement.\n"
                      ...
                      "You can type the full and shortened names, (kilometre, "
            ...
            measurement = in_list("Unit of measurement: ", measurement_units,
                                  "Error - please enter km, m, cm, mm, i, mi, "
                                  "ft or yd.").lower()
            history[7].append(measurement)
        # does calculations with shape and dimensions, displays area and/or
            # perimeter
            if area_perimeter == "Perimeter":
                perimeter = calculations(perimeter_formula[shape],
                                         "Perimeter: ", dimension_history, 6,
                                         measurement)
                history[5].append('n')
            elif area_perimeter == "Area":
                area = calculations(area_formula[shape], "Area: ",
                                    dimension_history, 5, measurement)
                history[6].append('n')
            else:
                area = calculations(area_formula[shape], "Area: ",
                                    dimension_history, 5, measurement)
                perimeter = calculations(perimeter_formula[shape],
                                         "Perimeter: ", dimension_history,
                                         6, measurement)
...
print()
print("Enter 'yes' for full calculation history "
...
full_display = in_list("Display whole dataframe?: ", yes_no,
